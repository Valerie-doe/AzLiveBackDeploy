...
def _analyzer_client_for_commande(commande: Commande) -> _ThreadPartialClient:
    """Client « virtuel » pour parser un nouveau MP : état du fil, pas l'ancien profil.

    Évite de bloquer une correction (nouvelle adresse / date) parce que le Client en base
    a encore les préférences d'une commande live ou article précédente.
    """
    partial = _ThreadPartialClient(commande.client)
    for key, value in _fields_from_commande_inbounds(commande).items():
        _apply_thread_field(partial, key, value)
    return partial


def _sync_client_from_thread(commande: Commande) -> None:
    """Aligne le profil Client sur les infos collectées pour CETTE commande (fil + reprise)."""
